...
JVM 测试覆盖智能输出选择、外放转耳机、耳机断连禁止外放降级、守卫动作顺序、路由信号静音重验、耳机验证过期、闹钟调度字段、快捷时间跨午夜、主页停用与活动响铃 ID 的匹配规则、助眠声无重置切换和铃声试听选择。模拟器回归覆盖冷启动、两页导航、闹钟编辑、全新安装精确调度、应用退出前台后由系统拉起响铃，以及无耳机助眠声播放与播放中换声。
