...
本文记录 `0.3.5-beta` 的工程入口；产品行为以 `hush-wake-prd.md` 为准，实体机安全验收以 `device-test-guide.md` 为准。

## 运行结构

- `HomeActivity` 只承载首次引导、“闹钟”和“助眠声”两个主页面；系统权限缺失、后台受限和当前输出状态以内联卡片呈现。每次保存或从列表重新启用闹钟后，主页重新检查精确闹钟、通知、全屏响铃、系统后台限制、App Standby Bucket 和电池优化豁免，按优先级弹出对应处理入口；电池优化使用应用级系统确认，不再跳到模糊的普通应用详情。厂商自启动没有统一可读 API，只能明确标为无法自动确认。`EditAlarmActivity` 通过小时/分钟双滚轮和三个相对时间快捷项编辑时间，点选铃声通过短生命周期 `PrivatePlaybackEngine` 会话即时试听；保存新建或编辑结果会自动启用，启停开关只保留在列表页。
...
JVM 测试覆盖智能输出选择、外放转耳机、耳机断连禁止外放降级、守卫动作顺序、路由信号静音重验、耳机验证过期、闹钟调度字段、快捷时间跨午夜、主页停用与活动响铃 ID 的匹配规则、响铃状态刷新策略、六项后台唤醒检查优先级、后台与待机受限和电池优化就绪判定、助眠声无重置切换和铃声试听选择。模拟器回归覆盖冷启动、两页导航、闹钟编辑、保存与重新启用后的电池优化系统确认、全新安装精确调度、应用仍在后台或缓存进程被回收后由 Android 进入有声状态并展示锁屏响铃页，以及响铃期间从桌面启动应用不崩溃。
