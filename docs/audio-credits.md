...
iOS 构建通过 `scripts/ios/prepare_resources.py` 将下列 14 个已有 OGG 转为同名 48 kHz、160 kbps AAC/M4A，供 AVFoundation 离线解码；不改变作者和原许可。署名、来源、修改说明与 Apache-2.0 正文随 iOS 应用及下载包分发，应用内“说明 → 音频来源与开源许可”可离线查看。
...
| 深夜海浪 | `ocean_waves.ogg` | [Sea and river wave sounds / VistulaShort.mp3](https://opengameart.org/content/sea-and-river-wave-sounds) | RandomMind | CC0 1.0 |
| 远方雷雨 | `distant_thunder.ogg` | [rbh thunder storm.wav](https://freesound.org/people/RHumphries/sounds/2523/)；采用 [AmbientSounds 固定提交](https://github.com/Muges/ambientsounds/blob/7ef9aefeeed93c37bca3cdb246a982dd1afce2f0/thunderstorm.ogg) 的无缝循环版本 | RHumphries | [CC BY 4.0](https://creativecommons.org/licenses/by/4.0/) |
...
`gentle_stream.ogg` 来自 mystiscool 的 `stream2.wav`：AmbientSounds 先将其制作成无缝循环，本项目再截取为约 120 秒并转为 48 kHz OGG；再分发时保留作者、标题、原始素材页、修改说明和 CC BY 4.0 链接。`ocean_waves.ogg` 取自 5 分钟、320 kbps 的 `VistulaShort.mp3` 中间 120 秒，衰减 10 dB 后转为 48 kHz 立体声 OGG；`distant_thunder.ogg` 保留 AmbientSounds 的 48 kHz 立体声无缝循环版本。AOSP 闹铃素材的目录许可声明见同一提交的 [Android.bp](https://android.googlesource.com/platform/frameworks/base/+/1cdfff555f4a21f71ccc978290e2e212e2f8b168/data/sounds/Android.bp)，许可正文随仓库保存在 [`licenses/Apache-2.0.txt`](licenses/Apache-2.0.txt)；CC0 音频无需署名，但仍保留来源以便审查。
