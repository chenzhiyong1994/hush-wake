...
- 闹钟编辑页提供六种真实钟铃、风铃与自然录音；点选即启动短试听，再点当前铃声可以重播。试听使用同一智能输出守卫，不单独绕过耳机验证。
...
- 主页关闭正在响铃的重复闹钟会同步停止该次响铃；已自动停用的一次性闹钟显示独立的“立即停止”操作。
...
- **用户影响**：用户需要授予精确闹钟、通知和蓝牙连接相关权限，并理解“无耳机时外放；检测到耳机时只允许耳机路径”。
...
| R3 | 技术 | 精确闹钟权限被撤销、重启后未恢复或厂商杀后台 | 中 | 高 | 启用时/启动时/权限变化时重排；接收开机事件；主页提示和测试闹钟 |
...
| 编号 | 规则类型 | 规则描述 |
| --- | --- | --- |
| RNG-R1 | 触发 | 验证通过后按统一 15 秒策略从静音渐强至完整系统媒体音量，不改变系统音量 |
