...
首次打开 → 说明“无耳机外放、有耳机守卫” → 用户确认 → 进入闹钟首页 → 页面按需提示必要权限、媒体音量和当前输出；扬声器/耳机连接变化会实时刷新顶部状态，播放时自动检查本次实际路由。后台唤醒缺项以页面内卡片持续展示，用户点击后才进入对应系统设置；保存或重新启用闹钟不打断当前操作。
