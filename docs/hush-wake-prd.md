...
> 平台扩展（2026-09-20）：新增原生 iOS 开源 beta，最低 iOS 17。既有 Android 需求继续适用于 Android；iOS 的适用边界以紧接本段的平台表为准，不能将下文 Android 后台精确唤醒或蓝牙兼容等级直接套用到 iOS。不进行 App Store 上架。

| iOS 首版范围 | 需求与边界 |
| --- | --- |
| 闹钟 | 最多 8 个、单次/周重复、编辑/启停/删除、1 分钟测试、固定一次 5 分钟稍后、最长 2 分钟；过期实例不补播；同一时刻只开启一个有声会话 |
| 前台 / 后台 | 前台有声闹钟；锁屏/后台仅系统无声通知，受权限和专注模式约束；不保证唤醒，不用静音音轨维持后台运行 |
| 输出 | 会话开始仅允许内置扬声器或唯一、明确识别的有线耳机；启动前后复核路由；蓝牙/USB 等无法证明为私密耳机时保持阻断 |
| 断连与中断 | 先增益归零再停止，不自动恢复、不在耳机会话中降级外放；零串音需要真机验收 |
| 助眠 | 复用现有 8 种录音，后台正常音频播放、5–120 分钟、0/15/30 秒渐隐；暂停与换声保留截止时间 |
| 隐私与反馈 | 本地 JSON、排除备份、无上传/账号/设备标识落盘；前台单次轻触反馈默认关闭，用户显式开启 |
| 分发与验收 | GitHub 公开源码、未签名 IPA、模拟器包和校验文件；真机需使用自己的身份签名；CI 和模拟器不解除实体机 beta 门禁 |

iOS 的实现、构建与实体机检查见 [iOS 实现说明](ios-implementation.md)，用户安装流程见 [iOS 下载说明](ios-install.md)。
...
| 2026-08-21 | 移除人工耳机验证页 | 顶部状态实时跟随扬声器/耳机连接；播放以双层静音自动检查本次实际路由，不再要求 10 秒测试或保存耳机验证记录 | 产品发起人 + Codex | JVM、Lint、API 36 模拟器与正式签名升级回归；实体耳机待复测 | v4.4-beta |
| 2026-08-21 | 将顶部音量入口替换为常驻自启动入口 | 输出卡不再提供重复的系统音量设置；待确认使用琥珀色风险态，已确认使用薄荷色确认态，两种状态均可进入厂商设置并在返回后重新确认 | 产品发起人 + Codex | JVM、Lint、API 36 模拟器与正式签名升级回归；厂商实体机入口待复测 | v4.5-beta |
| 2026-08-22 | 收敛闹钟首页焦点与响铃状态 | “下一次响铃”始终独占最近已开启或当前响铃的闹钟，提供启停和编辑入口；其余条目统一进入“其他闹钟”，关闭焦点后自动提升下一条 | 产品发起人 + Codex | JVM、Lint、API 36 双闹钟切换与后台响铃回到主页回归；实体机视觉待复测 | v4.6-beta |
| 2026-08-22 | 修正桌面图标安全留白 | B1-A 彩色与单色前景标识同步缩小并居中，在圆形、圆角矩形和厂商异形遮罩下保留稳定边距 | 产品发起人 + Codex | Lint、API 36 Pixel Launcher 圆形遮罩与桌面截图回归；厂商圆角矩形遮罩待复测 | v4.7-beta |
| 2026-09-01 | 阻止闹钟早于目标时间响铃 | 提前到达的系统或厂商广播不再被误判为准时触发，只重新登记原目标；补充 13:03 在 13:00 不得响铃的回归 | 产品发起人 + Codex | 聚焦回归、完整 JVM、Lint 与 Debug 构建通过；受影响厂商实体机待复测 | v4.8-beta |
...
产品采用智能输出：会话开始时没有耳机则允许系统媒体外放；检测到唯一耳机则进入严格耳机模式，播放器以双层静音启动并自动检查本次实际路由，匹配后才解除静音。耳机播放中断连或路由不确定时保持静音并用可选振动和通知兜底，不在同一会话中突然降级回扬声器。
...
- Android 16（API 36）开始可以枚举单个播放轨的全部实际路由设备；较旧版本只能获得较弱的单路由证据，因此必须标为兼容验证并保留实体机验收门禁。
...
- **经过**：用户创建 13:55 的私密闹钟，应用完成权限、耳机和媒体音量检查；到点自动验证本次实际路由，仅在匹配耳机后播放。
...
| 手工方案：系统闹钟 + 连接耳机 | 无需安装新应用 | 使用成本低 | 结果依赖系统/厂商路由，断连后可能改走扬声器 | 用本次实际路由检查和失败静音降低不确定性 |
...
4. 应用可检测并引导处理精确闹钟权限、通知权限、媒体音量为零、多耳机目标不唯一、重启后未重排等问题。
5. 用户可运行 1 分钟测试闹钟；耳机有声不依赖独立人工验证页，每次播放自动检查本次实际路由。
...
3. 70% 以上完成首次安装的用户理解顶部输出状态并创建至少 1 个闹钟。
...
| 3 | 就绪提示 | 在使用页面内提示精确闹钟、通知、媒体音量和实时输出状态 | P0 |
...
- 自动实际路由检查、1 分钟测试闹钟、环境健康检查、失败静音、断连阻断。
...
### 5.5 当前交付：0.4.8-beta 正式功能测试版

当前仓库已在技术切片基础上完成 P0/P1 首版：

- API 31+ 一次性/周重复闹钟、本地持久化、精确调度、系统事件重排、锁屏通知、停止、一次稍后提醒、最长响铃和振动兜底。
- API 33+ 使用闹钟应用允许的安装即授权精确调度能力；API 31–32 保留用户可管理的兼容能力。应用退出页面、进入后台或缓存进程被系统终止后，已登记闹钟仍由 Android 唤醒。主页检查精确闹钟、通知、全屏响铃、系统后台限制、App Standby Bucket 和电池优化豁免；缺项显示内联处理入口，保存或重新启用时不得自动弹窗或打开设置。厂商自启动没有统一读取 API，共享顶部输出卡始终保留状态入口，已确认后也允许重新进入厂商启动管理并修正人工确认；编辑页在未确认时显示风险，闹钟卡片不重复提示。
- 顶部输出卡片实时显示扬声器或耳机状态；独立 10 秒耳机验证页、人工确认门禁和设备验证记录已移除，每个播放会话都重新检查实际路由。
- 正式闹钟与八种真实环境录音共用智能输出守卫；助眠声支持暂停、通知控制、5–120 分钟定时、结束渐隐以及保留当前计时的即时换声，来源与许可见 `audio-credits.md`。
- 闹钟编辑页提供六种许可清晰的 AOSP 专业闹铃素材；点选即启动短试听，再点当前铃声可以重播。试听使用同一智能输出守卫，不绕过本次实际路由检查。
- 主界面只保留“闹钟”和“助眠声”。系统权限和输出状态按需内联提示，不提供独立可靠性、记录或设置页。
- 普通冷启动依次呈现 Android 系统 B1-A 图标和品牌声场过渡；桌面彩色与主题单色前景标识使用一致的安全留白，适配圆形、圆角矩形与厂商异形遮罩；界面恢复、通知跳转与活动响铃跳过装饰过渡。空闹钟列表只保留一个底部主创建入口，助眠声音卡不显示左上角文字角标。
- 两页分别以“几点叫醒你？”和“想听什么入睡？”作为任务导向标题，共用夜幕蓝黑背景、琥珀焦点、玻璃卡片和胶囊底部导航。闹钟页在“下一次响铃”中独占显示最近一次已开启闹钟，并提供开关和编辑入口；关闭后自动提升下一条，其余条目只显示在“其他闹钟”。当前响铃即使已消费一次性计划也继续占据焦点并提供停止入口，不得同时出现空焦点或在下方重复。助眠页用横向声音库、状态声波和直选胶囊呈现真实会话，不再呈现密集设置列表。
...
- 主页关闭正在响铃的焦点闹钟会同步停止该次响铃；当前响铃在焦点卡显示独立的“立即停止”操作；服务确认停止后主页立即刷新、移除该操作并提升下一条已开启闹钟。
...
| A3 | 用户接受旧版 Android 仅能提供兼容级单路由证据 | 若不接受，需要将 API 31–35 改为完全无声或提高最低系统版本 |
...
| C2 | Android 15 及以下无法对单播放轨枚举所有实际输出设备 | 采用兼容验证、每次检查单路由证据并保留实体机门禁；文案不得声称系统级绝对保证 |
...
| 兼容验证 | Compatibility Verification | 系统证据不完整，结合首选路由、当前单路由信息和设备事件形成的较弱保证 |
...
flowchart TD
    ...
    DEVICE -->|是且唯一| SILENT
    DEVICE -->|多个候选| BLOCK[保持静音<br/>振动/通知兜底]
...
| 预检中 | 全部输出条件通过 | 响铃中 | 输出守卫 | 唯一耳机、实际路由与音量均合格，或无耳机时实际路由为扬声器 | 解除应用播放器静音，记录验证级别 |
...
| 输出 | 顶部实时状态 | ✓ | — | 显示扬声器/耳机/阻断状态；播放时自动检查实际路由 |
...
首次打开 → 说明“无耳机外放、有耳机守卫” → 用户确认 → 进入闹钟首页 → 页面按需提示必要权限、媒体音量和当前输出；扬声器/耳机连接变化会实时刷新顶部状态，播放时自动检查本次实际路由。顶部输出卡常驻自启动状态入口，未确认时突出风险，已确认后仍允许重新进入厂商设置；其他后台唤醒缺项以内联卡片持续展示，用户点击后才进入对应系统设置；保存或重新启用闹钟不打断当前操作。
...
| 后台唤醒设置 | 标准检查通过/缺少具体能力/厂商状态待人工确认 | 用户点击内联入口后打开对应系统确认或厂商启动管理 | 顶部输出卡始终保留自启动入口；待确认为琥珀色、已确认为薄荷色；两种状态都可重设，返回后再次确认；不得在保存或启用时强制弹窗 |
| 媒体音量 | 可用/为零/过低 | 使用系统实体音量键调整 | 大于 0；页面不再提供独立音量设置入口；低于建议阈值只警告 |
...
| ONB-R3 | 触发 | 设备连接状态发生变化时，顶部输出状态应自动刷新；不得要求用户进入独立验证页面 |
| ONB-R4 | 约束 | 每次耳机会话都必须从双层静音开始并验证当前实际路由；不得自动提高系统媒体音量 |
...
| ALM-R7 | 约束 | 闹钟不得早于登记目标时间响铃；若系统或厂商提前投递触发广播，只重新登记原目标，不推进状态或开始播放 |
| ALM-R7 | 事实 | 卸载、强制停止、关机、无电属于应用无法保证的中断条件，帮助页必须明确说明 |
| ALM-R8 | 触发 | 新建或编辑页保存后自动启用闹钟；启停开关只在列表页提供 |
| ALM-R9 | 约束 | 主页必须复检六项 Android 标准唤醒能力并在列表顶部以内联方式提醒第一个标准缺项；顶部输出卡必须始终保留厂商自启动入口，未确认时以琥珀色显示待确认，已确认时以薄荷色显示已确认，两种状态均可点击，且从设置返回后必须再次确认并允许改回未确认。编辑页仅在未确认时提示风险，闹钟卡片不得重复展示启动检测状态。只有用户点击入口才打开系统设置，不得在保存或启用时强制弹窗，且不得宣称能绕过系统强行停止 |

#### 10.2.3 私密输出守卫与闹钟触发

##### 10.2.3.1 业务流程

到点事件 → 创建播放器但增益为 0 → 枚举当前兼容耳机 → 无耳机则进入系统媒体外放；有唯一耳机则设置首选设备并检查本次实际路由 → 通过后解除静音 → 持续监听设备/路由 → 路由信号先归零并重选 → 外放期接入耳机则转入自动耳机路由检查；耳机实际断连、路由不匹配或验证不确定则停止并执行振动/通知兜底。
...
| 兼容验证 | Android 12–15 / API 31–35 | 首选设备设置成功；本次播放器单路由信息与唯一耳机匹配；连接事件稳定 | “兼容验证：当前路由已匹配，系统能力有限” |
| 不可用 | 多个耳机目标不唯一、实际路由不匹配、路由超时、媒体音量为 0 或系统低于支持版本 | 不允许有声 | “已阻断：输出条件不满足” |
...
| 输出状态 | 扬声器播放/耳机播放/输出受阻 | 只读 | 随设备连接实时刷新；白噪音与闹钟共用智能输出守卫 |
...
- 本地只保存闹钟、必要偏好和当前会话摘要；不保存耳机名称、地址、哈希、人工验证记录或用户行为历史。
...
| 后台限制 | 系统或厂商电池策略阻止触发 | 检测后台限制、Restricted Standby Bucket 与电池优化豁免；厂商自启动保留人工确认 | 顶部常驻自启动状态入口，编辑页在未确认时补充风险；用户点击后才打开对应入口，明确区分自动检查与人工确认 |
...
| 后台受限、待机桶、电池优化与厂商自启动 | 判断 Android 标准机制是否会拦截后台触发，并保留厂商设置确认状态 | 首页持续检查；顶部常驻自启动入口，编辑页未确认时持续展示 | 缺项以内联提醒呈现；用户主动点击后才打开系统入口；厂商自启动已确认后仍可重设，不承诺强行停止后仍可唤醒 |
...
- P0 隐私问题需保存复现步骤、系统版本、耳机类型、顶部输出状态和是否出现扬声器声音；确认后立即暂停相关组合的有声能力。
...
| TBD-4 | 兼容验证模式是否允许正式闹钟有声，或 MVP 仅开放 API 36+ 强验证 | 5、7、10 | 产品发起人 + Android 开发 | 技术原型后 | 已决定：API 31–35 在本次单路由证据匹配唯一耳机后允许兼容有声，不设人工验证页，保留实体机验收门禁 |
