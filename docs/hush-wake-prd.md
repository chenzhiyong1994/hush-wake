...
- 3–5 个离线白噪音、关闭/15/30/45/60 分钟计时、0/5/15/30 秒渐隐。
...
- 正式闹钟与四种离线白噪音共用失败静音输出守卫；白噪音支持暂停、通知控制、最长 8 小时定时和结束渐隐。
- 首次引导、可靠性中心、最近 30 次或 30 天本地记录、默认值和本地数据清除均已实现；不存在账号、云同步或诊断上传路径。
- API 31/34/36 模拟器已覆盖冷启动、五页遍历与闹钟编辑；API 34 另覆盖系统精确触发，API 31/34 覆盖无耳机失败静音，API 36 覆盖强验证能力分支。实体耳机零泄漏仍按测试指南作为 beta 门禁。
...
| 状态 | 可用操作 | 配置可编辑性 | 规则 |
| --- | --- | --- | --- |
| 播放中 | 暂停、停止 | 当前会话配置只读 | 更改声音、增益或定时需停止后新建会话 |
...
| OUT-R2 | 约束 | 任何设备断连、路由变化、播放器重建或验证超时，处理顺序必须是“应用增益归零 → 停止”；本次会话不自动恢复有声 |
...
| 输出状态 | 仅已验证耳机 | 只读 | 白噪音与私密闹钟共用失败静音输出守卫 |
