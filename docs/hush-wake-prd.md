...
### 5.5 当前交付：0.3.4-beta 正式功能测试版

当前仓库已在技术切片基础上完成 P0/P1 首版：

- API 31+ 一次性/周重复闹钟、本地持久化、精确调度、系统事件重排、锁屏通知、停止、一次稍后提醒、最长响铃和振动兜底。
- API 33+ 使用闹钟应用允许的安装即授权精确调度能力；API 31–32 保留用户可管理的兼容能力。应用退出页面、进入后台或缓存进程被系统终止后，已登记闹钟仍由 Android 唤醒。首次成功调度后明确说明 Android 没有统一后台权限，并引导用户从应用详情检查电池、自启动或后台活动；系统明确标记后台受限时继续显示风险和设置入口。
...
首次打开 → 说明“无耳机外放、有耳机守卫” → 用户确认 → 进入闹钟首页 → 页面按需提示必要权限、媒体音量和当前输出；检测到未验证耳机时才提供隐私测试入口。首次成功调度闹钟后，独立说明 Android 没有统一后台权限，并引导用户检查应用详情中的电池、自启动或后台活动设置。
...
| 后台唤醒设置 | 已引导/待检查/系统明确受限 | 打开应用详情 | 用户可检查电池、自启动或后台活动；厂商状态无法统一读取 |
...
| ALM-R9 | 约束 | 首次成功调度后必须明确引导检查电池、自启动或后台活动；系统明确限制后台运行时不得展示“完全就绪”，并持续提供品牌无关的设置入口，但不得宣称能绕过系统强行停止 |
...
| 后台限制 | 系统或厂商电池策略阻止触发 | 检测系统后台受限状态；提供应用详情中的电池/自启动入口和测试闹钟，不直接申请不必要的电池优化豁免 | 标记兼容风险并提供“去解除”入口 |
...
| 后台受限状态与厂商自启动设置 | 判断系统是否会拦截后台触发；为无法标准化检测的厂商开关提供入口 | 闹钟首页检查；每个已启用闹钟保留应用详情入口 | 标记风险；不申请不必要的电池优化豁免，不承诺强行停止后仍可唤醒 |
