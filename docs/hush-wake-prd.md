...
- 正式闹钟与三种真实环境录音共用智能输出守卫；助眠声支持暂停、通知控制、最长 8 小时定时和结束渐隐，来源与许可见 `audio-credits.md`。
...
选择真实环境录音 → 设置定时与结束渐隐 → 按系统媒体音量播放 → 后台/锁屏继续 → 定时进入渐隐 → 停止；若闹钟触发，则助眠声优先结束并让出音频焦点。

##### 10.2.5.2 页面交互

| 字段/控件 | 默认值 | 类型 | 规则 |
| --- | --- | --- | --- |
| 声音卡片 | 上次选择或雨声 | 单选 | 三个有明确再分发许可的真实环境录音 |
