...
### 5.5 当前交付：0.3.3-beta 正式功能测试版

当前仓库已在技术切片基础上完成 P0/P1 首版：

- API 31+ 一次性/周重复闹钟、本地持久化、精确调度、系统事件重排、锁屏通知、停止、一次稍后提醒、最长响铃和振动兜底。
- API 33+ 使用闹钟应用允许的安装即授权精确调度能力；API 31–32 保留用户可管理的兼容能力。应用退出页面、进入后台或缓存进程被系统终止后，已登记闹钟仍由 Android 唤醒；系统明确标记后台受限时显示风险和设置入口，厂商自启动能力由用户从同一入口检查。
...
首次打开 → 说明“无耳机外放、有耳机守卫” → 用户确认 → 进入闹钟首页 → 页面按需提示必要权限、媒体音量和当前输出；检测到未验证耳机时才提供隐私测试入口。
...
| ALM-R9 | 约束 | 系统明确限制后台运行时不得展示“完全就绪”；需显示品牌无关的电池/自启动设置入口，但不得宣称能绕过系统强行停止 |
