...
### 5.5 当前交付：0.3.5-beta 正式功能测试版

当前仓库已在技术切片基础上完成 P0/P1 首版：

- API 31+ 一次性/周重复闹钟、本地持久化、精确调度、系统事件重排、锁屏通知、停止、一次稍后提醒、最长响铃和振动兜底。
- API 33+ 使用闹钟应用允许的安装即授权精确调度能力；API 31–32 保留用户可管理的兼容能力。应用退出页面、进入后台或缓存进程被系统终止后，已登记闹钟仍由 Android 唤醒。每次保存或重新启用闹钟都检查精确闹钟、通知、全屏响铃、系统后台限制、App Standby Bucket 和电池优化豁免；缺项弹出对应处理入口。厂商自启动没有统一读取 API，不得伪装成已自动验证。
...
| 后台唤醒设置 | 标准检查通过/缺少具体能力/厂商状态不可读 | 打开对应系统确认或设置 | 六项标准能力逐次复检；厂商自启动不得伪装成已验证 |
...
| ALM-R9 | 约束 | 每次保存或重新启用后必须复检六项 Android 标准唤醒能力并提醒第一个缺项；厂商自启动标为不可自动确认，系统明确限制后台运行时不得展示“完全就绪”，且不得宣称能绕过系统强行停止 |
...
有声验证通过或进入静音兜底 → 展示锁屏通知/响铃页面并同步主页当前会话 → 用户从任一入口停止或稍后提醒 → 结束音频、振动和前台状态 → 计算下一次计划。
...
| 后台限制 | 系统或厂商电池策略阻止触发 | 检测后台限制、Restricted Standby Bucket 与电池优化豁免；闹钟核心功能受影响时请求应用级电池优化豁免；厂商自启动保留人工确认 | 标记具体风险并提供对应处理入口 |
...
| 后台受限、待机桶与电池优化 | 判断 Android 标准机制是否会拦截后台触发 | 每次保存或重新启用闹钟；首页持续展示 | 缺项提醒并打开对应系统入口；厂商自启动不可读，不承诺强行停止后仍可唤醒 |
