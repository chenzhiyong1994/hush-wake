...
- 正式闹钟与四种离线白噪音共用智能输出守卫；白噪音支持暂停、通知控制、最长 8 小时定时和结束渐隐。
- 首次引导、可靠性中心、最近 30 次或 30 天本地记录、默认值和本地数据清除均已实现；不存在账号、云同步或诊断上传路径。
- 既有 API 31/34/36 模拟器覆盖冷启动、五页遍历与闹钟编辑；智能输出版本需重新覆盖无耳机外放、播放中接入耳机和耳机断连分支。实体耳机零串音仍按测试指南作为 beta 门禁。
