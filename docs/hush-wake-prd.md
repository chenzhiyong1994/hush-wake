...
- 6 个离线真实助眠录音、关闭/15/30/45/60 分钟计时、0/5/15/30 秒渐隐。
...
### 5.5 当前交付：0.3.6-beta 正式功能测试版

当前仓库已在技术切片基础上完成 P0/P1 首版：

- API 31+ 一次性/周重复闹钟、本地持久化、精确调度、系统事件重排、锁屏通知、停止、一次稍后提醒、最长响铃和振动兜底。
- API 33+ 使用闹钟应用允许的安装即授权精确调度能力；API 31–32 保留用户可管理的兼容能力。应用退出页面、进入后台或缓存进程被系统终止后，已登记闹钟仍由 Android 唤醒。每次保存或重新启用闹钟都检查精确闹钟、通知、全屏响铃、系统后台限制、App Standby Bucket 和电池优化豁免；缺项弹出对应处理入口。厂商自启动没有统一读取 API，应用按厂商尝试打开启动管理页并记录用户人工确认，不得伪装成自动验证。
- 隐私测试记录绑定当前耳机的带本机随机盐不可逆哈希，有效 90 天；Android 大版本、音频引擎版本或耳机变化后失效。
- 正式闹钟与六种真实环境录音共用智能输出守卫；助眠声支持暂停、通知控制、最长 8 小时定时、结束渐隐以及保留当前计时的即时换声，来源与许可见 `audio-credits.md`。
- 闹钟编辑页提供六种许可清晰的 AOSP 专业闹铃素材；点选即启动短试听，再点当前铃声可以重播。试听使用同一智能输出守卫，不单独绕过耳机验证。
- 主界面只保留“闹钟”和“助眠声”。系统权限和输出状态按需内联提示，不提供独立可靠性、记录或设置页。
- 两页分别以“几点叫醒你？”和“想听什么入睡？”作为任务导向标题，主操作使用卡片与直选控件，不再呈现密集设置列表。
- 时间设置使用小时/分钟双滚轮，并提供 15 分钟后、30 分钟后和 1 小时后三个快捷项。
- 闹钟和助眠声均完全跟随系统媒体音量；不向用户暴露应用增益、振动处置、稍后提醒和最长响铃等内部统一策略。
...
首次打开 → 说明“无耳机外放、有耳机守卫” → 用户确认 → 进入闹钟首页 → 页面按需提示必要权限、媒体音量和当前输出；检测到未验证耳机时才提供隐私测试入口。每次保存或重新启用闹钟后重新审计 Android 可读取的后台唤醒能力，缺项弹出对应系统处理入口。
...
| 后台唤醒设置 | 标准检查通过/缺少具体能力/厂商状态待人工确认 | 打开对应系统确认或厂商启动管理 | 六项标准能力逐次复检；厂商自启动只记录当前厂商的人工确认，不得伪装成自动验证 |
...
| ALM-R9 | 约束 | 每次保存或重新启用后必须复检六项 Android 标准唤醒能力并提醒第一个缺项；标准项通过后若当前厂商尚无自启动人工确认，必须继续提醒并打开厂商启动管理或电池设置；系统明确限制后台运行时不得展示“完全就绪”，且不得宣称能绕过系统强行停止 |
...
| 字段/控件 | 默认值 | 类型 | 规则 |
| --- | --- | --- | --- |
| 声音卡片 | 上次选择或雨声 | 横向单选 | 六个有明确再分发许可的真实环境录音；播放中点选立即切换，数量增加时不纵向堆叠 |
| 播放/暂停 | 停止 | 按钮 | 状态即时同步到通知 |
| 音量 | 系统媒体音量 | 系统能力 | 页面不提供增益滑杆；实体音量键可随时调整 |
| 定时器 | 30 分钟 | 单选 | 关闭、15/30/45/60 分钟；关闭仍受 8 小时硬上限约束 |
...
| 后台限制 | 系统或厂商电池策略阻止触发 | 检测后台限制、Restricted Standby Bucket 与电池优化豁免；闹钟核心功能受影响时请求应用级电池优化豁免；厂商自启动打开厂商入口并记录人工确认 | 标记具体风险并提供对应处理入口，明确区分自动检查与人工确认 |
...
| 后台受限、待机桶、电池优化与厂商自启动 | 判断 Android 标准机制是否会拦截后台触发，并保留厂商设置确认状态 | 每次保存或重新启用闹钟；首页持续展示 | 标准缺项打开对应系统入口；厂商自启动不可读，按厂商打开入口并要求人工确认；不承诺强行停止后仍可唤醒 |
