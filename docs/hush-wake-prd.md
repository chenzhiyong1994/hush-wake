...
### 5.5 当前交付：0.3.7-beta 正式功能测试版

当前仓库已在技术切片基础上完成 P0/P1 首版：

- API 31+ 一次性/周重复闹钟、本地持久化、精确调度、系统事件重排、锁屏通知、停止、一次稍后提醒、最长响铃和振动兜底。
- API 33+ 使用闹钟应用允许的安装即授权精确调度能力；API 31–32 保留用户可管理的兼容能力。应用退出页面、进入后台或缓存进程被系统终止后，已登记闹钟仍由 Android 唤醒。主页检查精确闹钟、通知、全屏响铃、系统后台限制、App Standby Bucket 和电池优化豁免；缺项显示内联处理入口，保存或重新启用时不得自动弹窗或打开设置。厂商自启动没有统一读取 API，应用在编辑页、列表顶部和已开启闹钟卡片显示风险，用户主动点击后才按厂商打开启动管理页并记录人工确认。
...
| ALM-R9 | 约束 | 主页必须复检六项 Android 标准唤醒能力并以内联方式提醒第一个缺项；当前厂商尚无自启动人工确认时，编辑页、列表顶部和已开启闹钟卡片必须持续提示退出应用后的唤醒风险。只有用户点击入口才打开系统设置，不得在保存或启用时强制弹窗，且不得宣称能绕过系统强行停止 |
